pipeline {
    ...
    stages {
        ...
        stage('SonarQube Analysis') {
            steps {
               withSonarQubeEnv('qube') {
                    sh ''' $SCANNER_HOME/bin/sonar-scanner -Dsonar.projectName=BETJA-STAGING -Dsonar.projectKey=BETJA-STAGING \
        ...
        stage('Dist.zip BackUp To S3 Bucket') {
            steps {
	        	withAWS(region: 'ap-southeast-1', credentials: '427499a7-0197-438e-bf25-87f4b6fb2702'){
                    sh 'ls -la'
	                sh 'aws s3 cp dist-$(date +"%d-%m-%y").zip s3://gml-jenkins-jpt/BETJA/STAGING/'
		        }
            }
        }	    
        ...
        stage('Dist Deployment to Staging Server') {
            steps {
               sshagent(['staging_do']) {
                    sh 'ssh -o StrictHostKeyChecking=no brian@128.199.105.148 rm -r /var/www/html/frontEnd/betja/*'
                    sh 'scp -r dist/* brian@128.199.105.148:/var/www/html/frontEnd/betja/'
                }               
            }
        }
