pipeline {
    ...
    stages {
        ...
        stage('SonarQube Analysis') {
            steps {
               withSonarQubeEnv('qube') {
                    sh ''' $SCANNER_HOME/bin/sonar-scanner -Dsonar.projectName=JKC-REBRAND-STAGING  -Dsonar.projectKey=JKC-REBRAND-STAGING \
        ...
        stage('Dist.zip BackUp To S3 Bucket') {
            steps {
	        	withAWS(region: 'ap-southeast-1', credentials: '427499a7-0197-438e-bf25-87f4b6fb2702'){
                    sh 'ls -la'
	                sh 'aws s3 cp dist-$(date +"%d-%m-%y").zip s3://gml-jenkins-jpt/JKC-REBRAND/STAGING/'
		        }
            }
        }	    
        ...
        stage('Dist Deployment to Staging Server') {
            steps {
               sshagent(['staging_do']) {
                    sh 'ssh -o StrictHostKeyChecking=no brian@128.199.105.148 rm -r /var/www/html/frontEnd/joka-redesign/*'
                    sh 'scp -r dist/* brian@128.199.105.148:/var/www/html/frontEnd/joka-redesign/'
                }               
            }
        }
        ...
        stage('Send Webex Notification') {
		    ...
		    env.GIT_COMMITTER_NAME = sh (script: 'git log -1 --pretty=%cn ${GIT_COMMIT}', returnStdout: true).trim()
                    def payload = """
                    {
                        "markdown": "Jenkins Pipeline: ${jobName}, Build number: ${buildNumber} has run with Status: SUCCESS. Please check url at https://stage-jkc.bogaminglab.com . The pipeline was triggered by: ${GIT_COMMITTER_NAME}"
                    }
                    """
                    def headers = ['Content-Type: application/json']
        ...
        stage('Invoke Logic App') {
            steps {
                script {
                    ...
                    {
                        "JobName": "${jobName}",
                        "Site": "https://stage-jkc.bogaminglab.com",
